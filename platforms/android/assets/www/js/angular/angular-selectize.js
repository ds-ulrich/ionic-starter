...
angular.module('selectize', []).value('selectizeConfig', {}).directive("selectize", ['selectizeConfig', function(selectizeConfig) {
  return {
    restrict: 'EA',
    require: '^ngModel',
    scope: { ngModel: '=', update: '=', config: '=?', options: '=?', ngDisabled: '=', ngRequired: '&' },
    link: function(scope, element, attrs, modelCtrl) {
      ...
      var setRefreshItems = function(newVal, oldVal) {
        angular.forEach(scope.options, function(opt){
            selectize.updateOption(opt.id, opt);
        });
      };
      ...
      settings.onInitialize = function() {
        ...
        scope.$watch('update', setRefreshItems);
